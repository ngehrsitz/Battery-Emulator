...
void init_Ethernet() {
  DEBUG_PRINTF("init_Ethernet: MDC=%d MDIO=%d POWER=%d phy_type=%d phy_addr=%d clk_mode=%d\n",
               ...
               (int)esp32hal->ETH_PHY_TYPE(), esp32hal->ETH_PHY_ADDR_NUM(), (int)esp32hal->ETH_CLK_MODE());
  ...
  const bool ok = ETH.begin(esp32hal->ETH_PHY_TYPE(), esp32hal->ETH_PHY_ADDR_NUM(), esp32hal->ETH_PHY_MDC_PIN(),
                            esp32hal->ETH_PHY_MDIO_PIN(), esp32hal->ETH_PHY_POWER_PIN(), esp32hal->ETH_CLK_MODE());
  if (!ok) {
